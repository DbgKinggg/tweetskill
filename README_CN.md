...
| 拉多少推文 | X API 费用（owned reads） | LLM 费用 | 合计估算 |
|-----------|--------------------------|---------|---------|
| 50 条     | $0.05                    | ~$0.01  | ~$0.06  |
| 100 条    | $0.10                    | ~$0.01  | ~$0.11  |
| 200 条    | $0.20                    | ~$0.01  | ~$0.21  |
| 500 条    | $0.50                    | ~$0.01  | ~$0.51  |
| 刷新已有 skill | $0.01–0.05          | ~$0.01  | 便宜很多 |

LLM 费用基于 Gemini Flash（默认，通过 OpenRouter），可用 `--llm` 换模型。
