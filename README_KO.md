...
[예시](#예시) · [할 수 있는 것들](#할-수-있는-것들) · [설치](#설치) · [작동 원리](#작동-원리) · [요금](#요금)
...
스킬 파일이 생기면 처음 생각보다 훨씬 많은 걸 할 수 있다.

### 내 목소리로 오리지널 트윗 쓰기

가장 기본적인 사용법. 스킬 파일을 에이전트에게 주고 주제만 말하면 된다.

```
myaccount-skill.md 읽고 Claude Opus 4.7 출시에 대한 트윗 써줘
```

```
내 스타일로 3가지 버전 써줘 — 내가 고를게
```

매번 "이모지 안 써" 또는 "결론 먼저"라고 설명할 필요 없다 — 스킬 파일에 이미 있다.

---

### 내가 쓴 것처럼 들리는 답글 쓰기

답글 달고 싶은 트윗을 봤는데 어떻게 쓸지 생각할 시간이 없을 때.

```
myaccount-skill.md 읽고 이 트윗에 내 말투로 답글 써줘: [트윗 붙여넣기]
```

에이전트가 내 답글 패턴(반박·동의·보완)을 활용해서 생성해준다.

---

### 다른 사람의 콘텐츠 전략 파악하기

경쟁자나 배우고 싶은 계정을 분석해서 왜 반응이 나오는지 파악한다.

```
analyze @naval — 인게이지먼트 높은 트윗의 공통점을 알려줘
```

스킬 파일 생성 후, 어떤 오프닝을 쓰는지, 어떤 주제가 반복되는지, 어떤 포맷은 절대 안 쓰는지가 보인다.
...
```
elonmusk-skill.md 와 vitalikbuterin-skill.md 읽고
AI 규제에 대한 트윗을 각 스타일로 써줘
```
...
`CLAUDE.md` 또는 에이전트 시스템 프롬프트에 추가:

```
내가 쓰는 모든 트윗과 답글은 기본적으로 myaccount-skill.md 스타일로 써줘
```

그 다음엔 그냥 "이거 트윗 써줘"라고 하면 끝이다.
